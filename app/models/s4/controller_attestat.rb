  ...
  class ControllerAttestat < S4::Resource
    ...
    def self.create_xml(rowset)
      ...
      rowset.collect do |row|
        if row.getParentRow.no_attestats
          next
        end
        properties = []
        properties << {:attrs => {:name => 'controller_id.id', :ref_id => -1 * row.parent_id.to_i, :ref_resource => :controller}, :value => -1 * row.parent_id.to_i}
      ...
      S4::Card.build_xml('controller_attestat', resources)
    end
  end
