...
parent_pdf.text_field("v_lice", 170, 656, 353, 15, :default => "#{@vars['ceo_position']}, #{@vars['ceo_fio']}")
parent_pdf.text_field("v_lice_to", 0, 633, 523, 15)

parent_pdf.text_field("ypolnomachivaet", 275, 616, 248, 15)
parent_pdf.text_field("ypolnomachivaet_to", 0, 592, 523, 15, :default => @vars['agent_position'])
...
parent_pdf.text_field("treider_telephones", 163, 373, 327, 15)
parent_pdf.text_field("treider_email", 190, 347, 300, 15)

parent_pdf.text_field("agent_podpis", 0, 310, 180, 15, :default => @vars['ceo_position'])
...
parent_pdf.text "#{"_"*87}", :leading => 4
parent_pdf.text "действующего на основании Устава, уполномочивает #{"_"*41}", :leading => 4, :align => :justify
parent_pdf.text "<sup>(занимаемая должность лица, Ф.И.О. представителя Участника торгов)</sup>", :inline_format => true, :indent_paragraphs => 300
parent_pdf.text "#{"_"*87}", :leading => 4
...
parent_pdf.text "объявлять (подавать, изменять, отзывать ранее поданные) заявки и заключать сделки, получать, предоставлять и подписывать документы и осуществлять иные действия, предусмотренные внутренними документами ЗАО «ФБ ММВБ», устанавливающими требования к организации торговли на фондовом рынке ЗАО «ФБ ММВБ». "
...
parent_pdf.text "<sup>(Ф.И.О. представителя Участника торгов)</sup>", :inline_format => true, :indent_paragraphs => 85
...
parent_pdf.text "\n"
parent_pdf.text "Контактные телефон Трейдера :______________________________________________________"
parent_pdf.text "\n"
...
parent_pdf.text "#{"_"*30}#{" "*65} ___________/ ___________", :leading => 4
parent_pdf.text "<sup>(Должность)</sup>#{" "*95}<sup>(Подпись)</sup>#{" "*15}<sup>Ф.И.О.</sup>", :inline_format => true, :indent_paragraphs => 70
