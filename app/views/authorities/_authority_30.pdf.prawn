...
parent_pdf.text_field("position_agent2234", 20, 275, 503, 15, :default => "#{@vars['agent_position']}#{@vars["nbsp_a"]} #{@vars['agent_fio']}")
...
parent_pdf.text_field("position_agent2244", 49, 151, 310, 15, :default => @vars['agent_fio'])
...
"
parent_pdf.text "Подпись_____________________________________________________ удостоверяю."
...
Настоящая Доверенность действительна до  ________________________________________________ ."
parent_pdf.text "<sup>(срок действия Доверенности прописью)</sup>", :inline_format => true, :align => :center
parent_pdf.text "Контактные тел. ________________________________________________________________________", :align => :justify
...
parent_pdf.text_field("as", 230, 118, 285, 15)
parent_pdf.text_field("sd", 90, 100, 430, 15)
parent_pdf.text_field("position_agent212", 70, 85, 400, 15)