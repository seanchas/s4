...
parent_pdf.text_field("position_agent212", 39, 682-move, 484, 15, :default => "#{@vars['ceo_position']}#{@vars["nbsp_c"]} #{@vars['ceo_fio']}")
...
parent_pdf.text_field("position_agent2234", 0, 275, 523, 15, :default => "#{@vars['agent_position']}#{@vars["nbsp_a"]} #{@vars['agent_fio']}")

parent_pdf.text_field("passport_ser", 74, 252, 40, 15)
parent_pdf.text_field("passport_num", 131, 252, 60, 15)
parent_pdf.text_field("passport_date", 232, 252, 291, 15)
...
parent_pdf.text_field("as", 232, 118-8, 291, 15)
parent_pdf.text_field("sd", 90, 100-14, 433, 15)
parent_pdf.text_field("position_agent212", 55, 85-16, 468, 15)
...
Настоящая Доверенность действительна до   ________________________________________________.", :leading => 4, :align => :right
...
parent_pdf.text "Подпись   #{"_"*78}", :leading => 4, :align => :right
